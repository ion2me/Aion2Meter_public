const createDetailsUI = ({
  ...
  const open = async (row, { force = false, restartOnSwitch = true } = {}) => {
    ...
    // [수정] 같은 유저 갱신(force=true)일 땐 깜빡임 방지를 위해 초기화 건너뜀
      const isRefresh = force && isSame;
  ...
  const close = () => {
    ...
    // [수정] 현재 열린 ID를 알 수 있게 getOpenedRowId 추가
    return { open, close, isOpen, render, getOpenedRowId: () => openedRowId };
};
