...
const createMeterUI = ({ elList, dpsFormatter, getUserName, onClickUserRow }) => {
  ...
  const renderRows = (rows) => {
    ...
    for (const row of rows) {
      ...
      if (damageContribution < 3) contributionClass = "error";
      else if (damageContribution < 5) contributionClass = "warning";

      if (view.prevContribClass !== contributionClass) {
        if (view.prevContribClass) view.rowEl.classList.remove(view.prevContribClass);
        if (contributionClass) view.rowEl.classList.add(contributionClass);
